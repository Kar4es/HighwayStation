/datum/quirk/chipped
	name = "Чипированный"
	desc = "Несколько лет назад вы поддались моде на скилл-чипы и вживили себе один из коммерчески доступных образцов."
	icon = FA_ICON_MICROCHIP
	value = 2
	gain_text = span_notice("Вы внезапно чувствуете, что вас чипировали.")
	lose_text = span_danger("Вы больше не чувствуете себя чипированным.")
	medical_record_text = "Пациент рассказал, как несколько лет назад увлёкся «гонкой за скилл-чипами», и теперь у него в голове вживлён бесполезный чип. Придурок."
	mail_goodies = list(
...
/datum/quirk/chipped/add_to_holder(mob/living/new_holder, quirk_transfer, client/client_source, unique = TRUE, announce = FALSE)
	...
	gain_text = span_notice("[installed_chip::name] гудит в вашей голове из-за информационной перегрузки.")
	lose_text = span_notice("Вы перестаёте чувствовать чип у себя в голове.")
	return ..()
