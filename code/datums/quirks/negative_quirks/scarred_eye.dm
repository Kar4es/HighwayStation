/datum/quirk/item_quirk/scarred_eye
	name = "Шрамы на глазу"
	desc = "Происшествие в прошлом лишило вас одного глаза, зато вы получили крутую повязку. Йо-хо-хо!"
	icon = FA_ICON_EYE_SLASH
	value = -2
	gain_text = span_danger("Даже спустя время глаз иногда ноет...")
	lose_text = span_notice("Ваше периферическое зрение расширяется примерно на тридцать процентов.")
	medical_record_text = "Пациент имеет серьёзные рубцовые повреждения на одном глазу, приведшие к частичной потере зрения."
	hardcore_value = 2
